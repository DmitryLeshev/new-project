...
class Kuz {
    ...
    encrypt(source_data) {
        ...
        return gostCrypto.subtle.importKey('raw', gostCoding.prototype.Hex.decode(this.key), 'GOST R 34.12-ECB', true, ['encrypt', 'decrypt'])
            .then(key =>
                gostCrypto.subtle.encrypt({
                    name: 'GOST R 34.12-ECB',
                    length: 128
                }, key, gostCoding.prototype.Chars.decode(source_data, 'utf8'))
            )
            .then(data => {
                    encoded_data = data;
    ...
    decrypt(crypted_data) {
        ...
        return new Promise((resolve, reject) => {
            gostCrypto.subtle.importKey('raw', gostCoding.prototype.Hex.decode(this.key), 'GOST R 34.12-ECB', true, ['encrypt', 'decrypt'])
                .then(function (key) {
                    return gostCrypto.subtle.decrypt({
                        name: 'GOST R 34.12-ECB',
                        length: 128
                    }, key, gostCoding.prototype.Hex.decode(data))
                })
                .then(dec => {
    ...
    stribogHash512(data) {
        return gostCrypto.subtle.digest({
            name: 'GOST R 34.11',
            length: 512
        }, gostCoding.prototype.Chars.decode(data, 'utf8'))
            .then(hash => gostCoding.prototype.Hex.encode(hash).replace(/\r|\n/g, ''))
    }
